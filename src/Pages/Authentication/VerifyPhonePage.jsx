...
import { verifyPhone, resendOtp } from "../../Redux/ActionCreators/AuthActionCreators";
...
export default function VerifyPhonePage() {
  ...
  useEffect(() => {
    if (auth.response?.result === "Done" && auth.response?.message === "Phone number verified successfully.") {
      navigate("/login");
    }
  }, [auth.response]);
  ...
  function postSubmit(e) {
    ...
    dispatch(verifyPhone({ phone, otp: code }));
  }

  function resendCode() {
    if (seconds > 0 || auth.loading) return;
  ...
  useEffect(() => {
    if (auth.response?.result === "Done" && auth.response?.message === "OTP sent successfully.") {
      setSeconds(120);
      setOtp(["", "", "", "", "", ""]);
          ...
          <div className="w-px-400 mx-auto">
            ...
            <form onSubmit={postSubmit}>
              ...
              <button type="submit" disabled={auth.loading} className="btn btn-primary d-grid w-100 mb-3">
                ...
                {auth.loading ? "Please wait..." : "Verify my account"}
              </button>
              ...
              <div className="text-center">
                ...
                {seconds > 0 ? (
                  ...
                  <button type="button" className="btn btn-link p-0 ms-1" onClick={resendCode}>
                    Resend
                  </button>
                )}
              </div>
