...
import { createVendorCategory, getVendorCategoryDetails, updateVendorCategory } from "../../../Redux/ActionCreators/VendorCategoryActionCreators";
...
export default function VendorCategoryFormPage() {
  ...
  useEffect(() => {
    if (editing) dispatch(getVendorCategoryDetails(id));
  }, [id]);
  useEffect(() => {
    if (editing && state.selected) setForm((v) => ({ ...v, ...state.selected }));
  }, [state.selected]);
  useEffect(() => {
    if (state.response?.result === "Done" && (state.response?.message === "Vendor category created successfully." || state.response?.message === "Vendor category updated successfully."))
  ...
  function postSubmit(e) {
    e.preventDefault();
    if (error) return;
    editing ? dispatch(updateVendorCategory({ id, data: form })) : dispatch(createVendorCategory(form));
  }
  return (
    <div className="container-xxl flex-grow-1 container-p-y">
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h4 className="fw-bold">{editing ? "Edit" : "Create"} Vendor Category</h4>
