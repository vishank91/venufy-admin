...
import {
    ...
    createRecord,
    updateRecordPath
} from "./APICallingService";

function getReason(data) {
    return typeof data.reason === "object"
        ? Object.values(data.reason || {}).join(", ")
        : data.reason || data.message || "Request failed.";
}
...
function* getSaga() {
    ...
    if (!response.response.ok || response.data.result === "Fail") {
        yield put({
            type: API_ERROR,
            payload: getReason(response.data)
        });
...
function* createSaga(action) {
    ...
    yield put({
        type: CREATE_PROFILE_RED,
        payload: response.data.data || response.data
    });
}

function* updateSaga(action) {
    let response = yield updateRecordPath(
        "vendors/profile",
        action.payload
    );
